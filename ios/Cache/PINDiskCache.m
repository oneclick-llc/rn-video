...
- (NSURL *)encodedFileURLForKey:(NSString *)key
{
    ...
    return [_cacheURL URLByAppendingPathComponent:key isDirectory:NO];
}
...
- (nullable NSURL*)fileURLForKeyAsync:(NSString *)key completion:(PINDiskCacheFileURLBlock)block
{
    if (block == nil) {
...
- (void)setObject:(id <NSCoding>)object forKey:(NSString *)key withAgeLimit:(NSTimeInterval)ageLimit fileURL:(NSURL **)outFileURL
{
    ...
    [self unlock];
    NSLog(@"⚽️ Cache.save.to %@", fileURL.absoluteString);
    if (outFileURL) {
        *outFileURL = fileURL;
    }
}
...
- (nullable NSURL*)fileURLForKey:(NSString *)key block:(nullable PINDiskCacheFileURLBlock)block
{
    return [self fileURLForKeyAsync:key completion:block];
}
